...
export class BookingSuccess implements OnInit {
  ...
  downloadPDF(): void {
    try {
      ...
      const primaryColor = [3, 169, 245]; // #03a9f5
      ...
      // Title - using simple text without Vietnamese characters that cause issues
      doc.setTextColor(255, 255, 255);
      doc.setFontSize(20);
      doc.setFont('helvetica', 'bold');
      doc.text('Flight Ticket', pageWidth / 2, 22, { align: 'center' });
      
      // Booking code section
      doc.setTextColor(textColor[0], textColor[1], textColor[2]);
      doc.setFontSize(16);
      doc.setFont('helvetica', 'bold');
      doc.text(`Booking Code: ${this.bookingData.bookingCode}`, 15, 50);
      
      // Flight information section
      doc.setFontSize(14);
      doc.setFont('helvetica', 'bold');
      doc.text('Flight Information', 15, 65);
      
      // Flight route and details
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(12);
      doc.text(`${this.bookingData.departure} to ${this.bookingData.destination}`, 15, 75);
      doc.text(`Departure: ${this.bookingData.departureTime}`, 15, 82);
      doc.text(`Arrival: ${this.bookingData.arrivalTime}`, 15, 89);
      doc.text(`Date: ${this.bookingData.flightDate}`, 15, 96);
      doc.text(`Airline: ${this.bookingData.airline}`, 15, 103);
      doc.text(`Flight Number: ${this.bookingData.flightNumber}`, 15, 110);
      
      // Passenger information section
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(14);
      doc.text('Passenger Information', 15, 125);
      ...
      tickets.forEach((ticket, index) => {
        ...
        doc.setFontSize(11);
        doc.setTextColor(textColor[0], textColor[1], textColor[2]);
        doc.text(`Ticket ${index + 1}`, 20, yPosition + 5.5);
        
        // Passenger details
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(10);
        doc.text(`Passenger: ${ticket.passenger.full_name}`, 20, yPosition + 12);
      ...
      doc.text('Generated by Cloudway System', pageWidth / 2, pageHeight - 15, { align: 'center' });
      doc.text(`Generated: ${new Date().toLocaleDateString('en-US')}`, pageWidth / 2, pageHeight - 10, { align: 'center' });
      
      // Save the PDF
      const fileName = `flight-ticket-${this.bookingData.bookingCode}-${new Date().getTime()}.pdf`;
      doc.save(fileName);
